...
class board:
    ...
    def possible_W_moves(self, threats=False):
        ...
        for rook in zip(i, j):
            rooks[tuple(rook)] = [
                move for move in self.move_rules_R(rook) if move != opponent_king_pos
            ]

        # Generate knight moves and filter out moves to the black king's position
        i, j = np.where(board_state == "N")
        for knight in zip(i, j):
            knights[tuple(knight)] = [
                move for move in self.move_rules_N(knight) if move != opponent_king_pos
            ]

        # Generate bishop moves and filter out moves to the black king's position
        i, j = np.where(board_state == "B")
        for bishop in zip(i, j):
            bishops[tuple(bishop)] = [
                move for move in self.move_rules_B(bishop) if move != opponent_king_pos
            ]
        ...
        for queen in zip(i, j):
            queens[tuple(queen)] = [
                move for move in self.move_rules_Q(queen) if move != opponent_king_pos
            ]
        ...
        for pawn in zip(i, j):
            if threats == False:
                ...
                pawns[tuple(pawn)] = [
                    move for move in pawn_moves if move != opponent_king_pos
                ]
            else:
                _, pawns[tuple(pawn)] = self.move_rules_P(pawn)
                pawns[tuple(pawn)] = [
                    move for move in pawn_threats if move != opponent_king_pos
                ]
        ...
        for i in range(8):
            for j in range(8):
                if self.current_board[i, j] == "P":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.PAWN, chess.WHITE)
                    )
                elif self.current_board[i, j] == "R":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.ROOK, chess.WHITE)
                    )
                elif self.current_board[i, j] == "N":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.KNIGHT, chess.WHITE)
                    )
                elif self.current_board[i, j] == "B":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.BISHOP, chess.WHITE)
                    )
                elif self.current_board[i, j] == "Q":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.QUEEN, chess.WHITE)
                    )
                elif self.current_board[i, j] == "K":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.KING, chess.WHITE)
                    )
                elif self.current_board[i, j] == "p":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.PAWN, chess.BLACK)
                    )
                elif self.current_board[i, j] == "r":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.ROOK, chess.BLACK)
                    )
                elif self.current_board[i, j] == "n":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.KNIGHT, chess.BLACK)
                    )
                elif self.current_board[i, j] == "b":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.BISHOP, chess.BLACK)
                    )
                elif self.current_board[i, j] == "q":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.QUEEN, chess.BLACK)
                    )
                elif self.current_board[i, j] == "k":
                    board.set_piece_at(
                        chess.square(j, 7 - i), chess.Piece(chess.KING, chess.BLACK)
                    )
                else:
